...
(async () => {
  ...
  await page.evaluate(() => document.querySelector('.jfp-pathway-lite')?.scrollIntoView());
  await new Promise(r => setTimeout(r, 500));
  await page.screenshot({ path: '/Users/jonerai/.openclaw/workspace/tmp/training-pathway-mobile.png', fullPage: false });
  await page.setViewport({ width: 1440, height: 1200, deviceScaleFactor: 1 });
  await page.goto('http://127.0.0.1:4325/training/', { waitUntil: 'networkidle2', timeout: 60000 });
  await page.evaluate(() => document.querySelector('.jfp-pathway-lite')?.scrollIntoView());
  await new Promise(r => setTimeout(r, 500));
  await page.screenshot({ path: '/Users/jonerai/.openclaw/workspace/tmp/training-pathway-desktop.png', fullPage: false });
  await browser.close();
})();
