...
(async () => {
  ...
  await page.evaluate(() => document.querySelector('#choose-path')?.scrollIntoView());
  await new Promise(r => setTimeout(r, 500));
  await page.screenshot({ path: '/Users/jonerai/.openclaw/workspace/tmp/training-phase2-mobile.png', fullPage: false });
  await page.setViewport({ width: 1440, height: 1000, deviceScaleFactor: 1 });
  await page.goto('http://127.0.0.1:4325/training/', { waitUntil: 'networkidle2', timeout: 60000 });
  await page.evaluate(() => document.querySelector('#choose-path')?.scrollIntoView());
  await new Promise(r => setTimeout(r, 500));
  await page.screenshot({ path: '/Users/jonerai/.openclaw/workspace/tmp/training-phase2-desktop.png', fullPage: false });
  await browser.close();
})();
