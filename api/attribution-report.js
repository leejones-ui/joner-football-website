...
import { getAnonymousAggregates, listReliableSales } from './_reliability-ledger.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const reliableSales = await listReliableSales()
    const legacySales = await listSales()
    // The reliability ledger is canonical once it contains authoritative
    // webhook/API payment records. Legacy rows remain a migration fallback.
    const sales = reliableSales.length ? reliableSales : legacySales
    const aggregates = await getAnonymousAggregates()
    const rows = []
    for (const id of ids) {
