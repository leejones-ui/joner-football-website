...
export function extractAttribution(data) {
  ...
  return {
    ...
    utm_term: clean(firstValue(data, 'utm_term'), 180),
    encoded_source: decoded.encoded_source,
  }
}

export function extractMetaIdentity(data, eventTime = Math.floor(Date.now() / 1000)) {
  const fbp = clean(firstValue(data, 'fbp') || firstValue(data, '_fbp'), 240)
  let fbc = clean(firstValue(data, 'fbc') || firstValue(data, '_fbc'), 500)
  const fbclid = clean(firstValue(data, 'fbclid'), 500)
