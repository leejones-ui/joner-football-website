...
const metaWithoutAdIds = classifyAttribution({ journey: { journey_id: id, last_touch: { utm_source: 'facebook', utm_medium: 'paid_social', fbclid: 'fb-click-only' } }, payment: { journey_id: id, paid_at: '2026-08-12T00:00:00.000Z' }, now: Date.parse('2026-08-12T01:00:00.000Z') })
assert.equal(metaWithoutAdIds.classification, 'meta', 'a proven Meta click must not become unknown only because ad ids were not preserved')
...
assert.ok(ATTRIBUTION_CLASSES.has('meta'))
...
const merged = mergedSales([], [
  ...
  { sale_id: 'reconcile:99', payment_id: 'ch_1', uscreen_user_id: '123', customer_reference: '123', customer_name: 'Test', plan: 'Max - Annual', amount: 249.99, currency: 'USD', kind: 'payment', paid_at: '2026-08-14T00:00:00Z', acquisition: 'unknown' },
...
const authoritativeWins = mergedSales(
  [{ sale_id: 'legacy:pay-authoritative', payment_id: 'pay-authoritative', uscreen_user_id: '123', acquisition: 'unknown', amount: 10 }],
  [{ sale_id: 'payment:pay-authoritative', payment_id: 'pay-authoritative', uscreen_user_id: '123', acquisition: 'facebook', amount: 10, currency: 'AUD' }],
)
...
assert.equal(mergedSales(
  [{ payment_id: 'same-provider-payment', uscreen_user_id: '123', kind: 'payment', acquisition: 'facebook' }],
  [{ payment_id: 'same-provider-payment', uscreen_user_id: '123', kind: 'refund', acquisition: 'facebook', amount: -10 }],
).length, 1)
...
assert.equal(mergedSales([], [{ payment_id: 'same', uscreen_user_id: 'one' }, { payment_id: 'same', uscreen_user_id: 'two' }]).length, 2, 'Never deduplicate different customer identities')
