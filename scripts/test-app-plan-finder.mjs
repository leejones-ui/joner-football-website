...
assert.equal(result({ content: ['message-lee'] }).id, 'plus')
assert.equal(result({ content: ['message-lee','raw'] }).id, 'max')
assert.deepEqual(result({ goals: ['lee-coaching'] }), { needs: 'coaching' })
assert.equal(result({ goals: ['lee-coaching'], coaching: 'include' }).id, 'max')
assert.equal(result({ goals: ['lee-coaching'], content: ['raw'] }).id, 'max')
